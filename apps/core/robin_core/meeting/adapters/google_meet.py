...
class GoogleMeetAdapter:
    ...
    async def join(self) -> None:
        ...
        await self.mute()
        await self._click_with_recovery("join_button", MEET_SELECTORS["join_button"], self.config.prejoin_timeout_ms)
        if not await self._page().is_visible(MEET_SELECTORS["joined_signal"], self.config.admission_timeout_ms):
    ...
    async def leave(self) -> None:
        if self.meet_page and self.state not in {MeetingState.IDLE, MeetingState.ENDED}:
            try:
                await self._click_with_recovery("leave_button", MEET_SELECTORS["leave_button"], 3_000)
            except Exception:
                pass
    ...
    async def start_presenting(self, url: str) -> None:
        ...
        if self.meet_page:
            ...
            await self._click_with_recovery("present_button", MEET_SELECTORS["present_button"], 5_000)
    ...
    async def stop_presenting(self) -> None:
        if self.meet_page:
            try:
                await self._click_with_recovery("stop_presenting_button", MEET_SELECTORS["stop_presenting_button"], 3_000)
            except Exception:
                pass
        self.presenting = False
        self.state = MeetingState.LISTENING
    ...
    async def _click_with_recovery(self, action: str, candidates, timeout_ms: int) -> str:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                if attempt > 1:
                    self._record_recovery(action, attempt, recovered=True, error=str(last_error or ""), page=page)
                return result
            except Exception as exc:
                last_error = exc
                screenshot_path = await self._capture_recovery_screenshot(action, attempt, page)
                self._record_recovery(action, attempt, recovered=False, error=str(exc), page=page, screenshot_path=screenshot_path)
                if attempt >= attempts:
                    raise
    ...
    async def _capture_recovery_screenshot(self, action: str, attempt: int, page: PageDriver) -> str | None:
        try:
            self.config.recovery_screenshot_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
            path = self.config.recovery_screenshot_dir / f"{timestamp}_{action}_attempt{attempt}.png"
