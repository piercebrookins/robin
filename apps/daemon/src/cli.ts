...
const checks: Array<[string, () => Promise<string>]> = [
  ...
  ["OpenAI model access", async () => { const key = (await exec("security", ["find-generic-password", "-a", process.env.USER ?? "", "-s", "com.robin.agent.OPENAI_API_KEY", "-w"])).stdout.trim(); const models = [process.env.ROBIN_OPENAI_MODEL ?? "gpt-5.6", process.env.ROBIN_REALTIME_MODEL ?? "gpt-realtime-2.1"]; for (const model of models) { const response = await fetch(`https://api.openai.com/v1/models/${encodeURIComponent(model)}`, { headers: { Authorization: `Bearer ${key}` }, signal: AbortSignal.timeout(10_000) }); if (!response.ok) throw new Error(`${model} returned ${response.status}`); } return `${models.join(" and ")} available`; }],
  ...
  ["Workspace capture", async () => { const frame = await new NativeDesktopHarness(process.env.ROBIN_HELPER_SOCKET ?? "/tmp/robin-helper.sock", Number(process.env.ROBIN_WORKSPACE_DISPLAY ?? 1)).screenshot(); if (frame.mime !== "image/png" || frame.data.length < 10_000) throw new Error("ScreenCaptureKit returned an empty frame"); const actual = `${frame.width}x${frame.height}`; const expected = process.env.ROBIN_EXPECTED_CAPTURE; if (expected && actual !== expected) throw new Error(`${actual}; expected ${expected}`); return `${actual} PNG from display ${frame.displayId}${expected ? " (matches configured workspace)" : ""}`; }],
